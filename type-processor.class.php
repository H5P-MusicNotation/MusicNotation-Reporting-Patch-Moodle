...
abstract class TypeProcessor {
  ...
  public function generateReport($xapiData, $disableScoring = false, $allowStyles = false) {
    ...
    //check if string is base64 encoded (if so, it is probably a MusicNotation Response)
    // it should have the mei namespace in it
    $meiNamespace = "http://www.music-encoding.org/ns/mei";
    $verovio = "Engraved by Verovio";
    $decodedResponse = base64_decode($this->getResponse($xapiData), true);
      ...
      strpos($decodedResponse, $verovio) !== false){
        ...
        try{ // if there is no valid html, this block will be omitted
          ...
          if($xpath->query('//*[@id="interactionOverlay"]')->length == 0){
            throw new Exception("svg invalid");
          }

          $query = '//*[@id="interactionOverlay"]/*[not(@id="annotationCanvas")]';
          $nodes = $xpath->query($query);

          // Delete all children of InteractionOverlay, except annoationCanvas
          if($nodes->length == 0){
            throw new Exception("svg invalid");
          }
